...
def read_subG(dataset, subG, N):
    """ ADDED: read the input subgraph node list and cast into scipy.sparse.csr_matrix format """

    A = sp.lil_matrix((N,N), dtype='d')

    cx = sp.coo_matrix(dataset)
...
def get_subgraphs(C, N, A):
    """ ADDED: For K=2, return the bipartition """
    lD, _ = eigsh(A, k=1, which='LA')
    sD, _ = eigsh(A, k=1, which='SA')
    X = sp.lil_matrix((N,2), dtype='d')
    for i,c in enumerate(C):
        if c>-1: X[i,c-1] = 1
    X = X.tocsr()
    S_1 = []
